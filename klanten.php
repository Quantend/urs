...
// Verwerken van verwijdering van klant
if (isset($_GET['action']) && $_GET['action'] === 'delete' && isset($_GET['id'])) {
    // Verkrijg de klant ID vanuit de URL-parameter
    $klantID = $_GET['id'];

    // Verwijder de klant uit de database
    $sql = "DELETE FROM klanten WHERE Klanten_ID = '$klantID'";
    if (mysqli_query($conn, $sql)) {
        echo "Klant is succesvol verwijderd.";
    } else {
        echo "Fout bij het verwijderen van de klant: " . mysqli_error($conn);
    }
}
...
<body>
    <nav>
        ...
        <ul>
            ...
            <li><a class="active" href="klanten.php">Klanten</a></li>
		...
		<table id="klantenTable">
			...
			<?php
				while($row = mysqli_fetch_assoc($result)) {
					echo "<tr><td>".$row["Klanten_ID"]."</td><td>".$row["Bedrijfsnaam"]."</td><td>".$row["Voornaam"]."</td><td>".$row["Tussenvoegsel"]."</td><td>".$row["Achternaam"]."</td><td>".$row["Geboortedatum"]."</td><td>".$row["Email"]."</td><td>".$row["Telefoonnummer"]."</td><td>".$row["Adres"]."</td>";
					echo "<td><a href='?action=delete&id=".$row["Klanten_ID"]."'><i class='fas fa-trash'></i></a></td></tr>";
				}
			?>
		</div>
		</table>
